...
nastaje radi ljudske pogreške te se ti propusti manifestiraju u		% Zašto je u navodnicima ljudska pogreska? - ne znam zašto mi se to činilo potrebnim, ispravljeno
...
memoriji. Na taj način napadačima je značajno otežan postupak			% Pronalaženje ranjivosti ili iskorištavanje? - Iskorištavanje
...
\begin{enumerate}
\setcounter{enumi}{1}
			% Izbacio bi ovo, Uvod se nikada ne opisuje. -Izbačeno
...
sustava, kao i mehanizmi zaštite koji djeluju kao programi u			% Mislite u sklopu štićene aplikacije? -Tu sam konkretno mislio na ASLP koji ima korisnički program unutar kojega se vrši permutacija
...
\item Zaključak: Sadrži sažetak o svemu dotada napisanom.			% Zaključak nije pregled, to je sažetak!
\end{enumerate}
...
\item NT zaglavlje
	...
	\item Tablice
	\end{itemize}
\item Zaglavlje sekcija									% Doista idu prvo sva zaglavlja, pa svi podaci? Ili su zaglavlje+podaci u paketu? -Da, idu prvo sva zaglavlja i onda podaci. Nakon opcionalnog zaglavlja obavezno dolaze prvo sva zaglavlja sekcija i tek onda sadžaj sekcija.
\item Podaci sekcija
\end{itemize}
...
ostvariti. U nastavku su objašnjena oba pojma.			% Rekao bi da su objašnjeni, a ne da su dane informacije
...
učitana u memoriju (\emph{0x400000}). Ta adresa naziva se	% Ta adresa, bez virtualna? -Ispravljeno
virtualnom adresom. Na ispisu \ref{lst:primjer_rva}				% Sve ``odsječke koda'' zamijeniti s ``ispis'' - Riješeno
...
(\emph{0x8B 0x0D}) sačinjavaju operacijski kod instrukcije dok				% Krivi oktet! -ispravljeno
...
Svaka \emph{IMAGE\_BASE\_RELOCATION} struktura sastoji se od dva			% Ovo mi nije jasno!? -probao sam bolje objasniti u nastavku
...
\emph{IMAGE\_DOS\_HEADER} strukture jer pokazuje na početak				% Kako se računa taj početak? -Opisano u nastavku odlomka
...
\begin{itemize}
\item PeView \citep{peview}: Alat koji pregledno prikazuje strukturu bilo koje			% U literaturu staviti linkove na alate i pozvati se na reference!
...
\caption{Primjer djelovanja ASLR-a izmađu dva pokretanja programa}					% Primjer djelovanja ASLR-a između...
\label{fig:aslr} 
\end{figure} 
...
\item Sklopovski podržan DEP: Detektira pokušaj izvršavanja programskog koda
 na stogu ili gomili te generira iznimku.		% Detektira pokušaj izvršavanja (nečega) na stogu...
...
osim ako neka od njih eksplicitno ne sadrži programski			% neizvršive...
kod. Ovaj oblik DEP-a oslanja se na operacijski sustav kako bi označio			% Procesor detektira pokušaj, a OS je taj koji označava...
...
u sklopovlju te način kako se označavaju neizvršive stranice u virtualnoj memoriji.			% te kako se označavaju neizvršive stranice...
...
Pod pojmom krupne permutacije podrazumijeva se prvi od dva koraka u			% ``nalazi se'' -> podrazumijeva se
...
sadrže virtualne i fizičke adrese podatkovnih i izvršnih			% Bez ``od''
...
se ASLP značajno razlikuje od modula za promjenu koda jer ima			% Sto je ``modul za promjenu''? -ispravljeno
...
Memorija za učitavanje vanjskih biblioteka				% mmap( je jezgrina funkcija, a ne ``memorija za ucitavanje vanjskih biblioteka''!) -uklonio sam taj dio
...
najviše koristili tijekom 90-ih godina prošlog stoljeća. 				% To nisu ljudi ili bica koji ``dozivljavaju vrhunac''!

Pod pojmom korupcije memorije misli se na one situacije u kojima
se sadržaj memorije nenamjerno mijenja. Najčešći uzrok takvim				% Mislim da se pise ``nenamjerno''
...
pogrešaka u radu s memorijom. 									% Svuda treba koristiti izraz ``pogresaka u radu s memorijom''!
