...
\begin{enumerate}							% I ovdje se trebaju ispraviti velika slova na pocetku rijeci!
\setcounter{enumi}{1}
...
\item Zaključak: Sadrži sažetak o svemu dotada napisanom.			% O ovome moramo jos raspraviti, nije dobro...
\end{enumerate}
...
strukture prikazan je u odsječku koda \ref{lst:imgSectionHeader}.				% Rekao sam da SVUDA promijenite ``odsjecak'' u ispis!
...
se ASLP značajno razlikuje od modula za promjenu koda jer ima			% Sto je ``modul za promjenu''? -ispravljeno - Jel' to PErmutator?
...
je ono detaljnije opisano u samom članku u ovom mehanizmu zaštite \citep{aslp}.
...
\item Između izvora memorijske korupcije i manifestacije istoga u		% Svuda koristite izraz ``korupcija memorije''
...
memorije tretira se kao "smeće". Korištenje					% Ne ``tretira'' se kao smece, jer da se tretira onda bi se pocistilo. Tretira se kao vazeci podaci!
...
spremnika): U ovom slučaju radi se o jednoj od najčešćih			% Recite ``cestima'' ili navedite referencu gdje se argumentira da su to doista najcesci slucajevi!
...
koji mogu izazvati ne definirano ponašanje programa. Ranjivost				% Ovo se pise zajedno! Vec sam vas upozorio na te pogreske i da ih popravite po cijelom tekstu!
preljeva spremnika moguće je iskoristit kada napadači žele				% iskoristitI
izvršiti neki strani programski kod ili kako bi se promijenio				% strani -> dodatni
...
jezici nemaju ugrađeni oblik zaštite koji bi kontrolirao				% bez ``oblik''
...
"AAAAAAAAAAAAAAAA0835C080" pri čemu adresa \emph{0x80C03508}				% To nije dobro, 08... je ASCII niz, a ne adresa!
...
sprječava izvršavanje ubačenog programskog koda. Ukoliko je DEP				% sprjecava ili sprIjecava? Dajte koristite hashek (ili kako se zove) i provjerite pravopis
...
potreba. Maksimalna veličina gomile određena je količinom				% Doista? Nije li ogranicena virtualnim prostorom?
...
vračaju 4 \emph{indexa}. Važno je naglasiti kako se alocirani			% To je meko ć! Jos jednom, spell checker!
blokovi zauzimaju slijedno, tj. jedan iza drugoga kako je
prikazano na slici \ref{fig:heap_2_allocated_blocks}.

% Na ovoj slici ima pojam ``Pokazivac na podatke''! Sto je to? Od kuda to?
\begin{figure}[!ht]
\centering
\setlength\fboxsep{0pt}
\setlength\fboxrule{0.5pt}
...
traženoj veličini uvečanoj za veličinu zaglavlja.				% Meko c!

\pagebreak									% Vec sam rekao, bez pagebreak-ova... pustite Latex da sam odreuje gdje ce lomiti stranice!

% Na ovoj slici imate ``Kolicina indexa'', to promijenite u BROJ indeksa ili BROJAC indeksa ili tako nesto
\begin{figure}[!ht]
\centering
\setlength\fboxsep{0pt}
\setlength\fboxrule{0.5pt}
...
% Za ovu sliku dati referencu kako bi se mogao pronaci detaljniji opis!
\begin{figure}[!ht]
\centering
\setlength\fboxsep{0pt}
\setlength\fboxrule{0.5pt}
...
(\ref{fig:heap_2_allocated_blocks}). Na liniji 8 vidljivo je kako			% Dodati Slika, jer kada je u tekstu samo broj ne zna se o cemu se radi! Dodatno, nije NA liniji, vec U liniji!
se blok \emph{m2} oslobađa te se informacije o bloku pohranjuju u
odgovarajući indeks unutar \emph{FreeLists} strukture.	Na liniji			% Opet, U liniji\ldots popraviti svuda
...
kao argument Format funkcijama kako bi iste mogle procesirati				% ovdje je ``Format funkcija'', a malo kasnije je ``format funkcija''. Inace, pise se malim slovom, to nije ime
...
korišteni format parametri vidljivi su u tablici 					% navedeni su
...
u konačnici mogli srušiti program ili izvršiti strani kod.				% Svuda promijeniti ``strani'' u dodatni (ili maliciozni)!

Najčešće korišteni format parametri koji se koriste prilikom				% Opis ubaciti u tablicu, a ovdje ih samo popisati! Nema potrebe i tablica i to!
...
mehanizme zaštite protiv napada usmjerenih na memoriju. Sve te			% Napadi koji iskoristavaju krivo upravljanje memorijom
...
sigurni kako bi se taj oblik napada u potpunosti iskorijenio.			% Nedovoljno dobri

U prethodnom poglavlju(\ref{sct:rop2}) pokazali smo i kako ASLR			% Opet, razmak prije zagrade
...
kako se određeni interesni dijelovi programskog koda nalaze na			% ``interesantni'', ili ``zanimljivi''
...
poznavati format PE datoteka(poglavlje \ref{sct:peFormat}). Tu se		% Razmak!
...
\label{sct:instrucionGraph}.							% Kriva referenca!
...
Github: https://github.com/bhumic/PErmutator					% Referenca, ne link u tekstu!
...
konačnici izbor se sveo na dvije biblioteka:					% bibliotekE
\emph{Distorm}\citep{distorm} i
\emph{BeaEngine}\citep{bea_engine} pri čemu je pobjedu uzeo			% Nisu se medusobno borili!
\emph{Distorm}.

\emph{Distorm} je izuzetno moćna i brza disasembler biblioteka
koja nudi podršku za dosta programskih jezika osim C/C++. Moguće		% ... osim C/C++ koji se koristi u ovom radu.
je biblioteku koristiti i za projekte u Pythonu, Javi te C\#-u.
Također, osim jednostavne pretvorbe sirovih okteta u smisleni
tekstualni zapis, \emph{Distorm} cijeli disasemblerski postupak			% ``tekstualni zapis'', nije li bolje u ``asemblerski'' ili nesto slino? Tekstualni je i niz okteta...
...
stranicama projekta\citep{distorm}. 						% Razmak!
...
promjenu koda, ovo poglavlje sadrži i ostale načine na koje je navedeni graf		% opisuje i druge nacine
moguće primijeniti. 									% prOmijeniti?
...
unutar datoteke \emph{Node.h} čiji najvažniji dijelovi se nalaze na odsječku
koda \ref{lst:node}. 
...
\item \emph{FindChild(DWORD)offset}: Funkcija koja vraća pokazivač na čvor			% Greska!
dijete kojemu odgovara argument \emph{offset}.
...
sljedeći primjer. Na odsječku koda \ref{lst:PE_instructions} nalazi se dio
...
Kao što je vidljivo na odsječku koda, postoje dvije instrukcije
...
i nalazi se u posljednjem, 21. redu odsječka koda
...
generirati iz navedenog odsječka koda prikazan je na slici
...
Zamislimo sada sljedeći scenarij: Neka se dani odsječak koda
preda kao ulazni podatak u modul za permutaciju te neka se
permutacija odvija na način da se blok na odsječku \emph{0x0}
zamijeni sa blokom na odsječku \emph{0x37} (radi se o dvama
...
adresa pokazuje na 12. redak u odsječku koda
...
odsječku \emph{0x37}. Kako bi nova datoteka bila ispravna,
...
odsječka koda \ref{lst:PE_instructions} je postavljanje
